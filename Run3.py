...
while True:
		...
		for line in temp:
			...
			if "!howto" in message:
				messageTemp = "To request a song for Brave New Faves, wait until Kathleen asks for suggestions, then "
				messageTemp = messageTemp + "drop a YouTube link in chat, along with the artist's name, song title, "
				messageTemp = messageTemp + "and a brief description hyping your request."
			...
			if ("kathleen_lrr" in user or "freshprinceofbeleren" in user) and "!printplaylist" in message:
				with io.open(playedlist, 'w', encoding="utf-8") as f:
					for line in lst:
						f.write(("%s\n\n" % line))
					f.close()
				break
