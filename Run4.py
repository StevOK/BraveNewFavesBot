...
#Here I come up with unique names for text files to be created by the bot
...
#Error message if someone tries typing !spotify before pterodactal sets it
spotifylist = 'No spotify playlist set'
...
while True:
		...
		for line in temp:
			...
			if ("youtube.com" in message or "youtu.be" in message) and ("bravenewfavesbot" not in user) and ("playlist" not in message):
			...
			#When command !howto is typed in chat, will print instructions for requesting a song	
			if "!howto" in message:
				messageTemp = "To request a song for Brave New Faves, wait until Kathleen asks for suggestions during "
				messageTemp = messageTemp + "the last song, then drop a YouTube link in chat, along with the artist's name, "
				messageTemp = messageTemp + "song title, and a brief description hyping your request."
				sendMessage(s, messageTemp)
				
			#When !bot command is typed in chat, will print the URL for the bot's chat.	
			if "!bot" in message:
				messageTemp = "Hi, I'm BraveNewFavesBot. Check out my chat window to see all tonight's requests: "
				messageTemp = messageTemp + "https://www.twitch.tv/popout/bravenewfavesbot/chat"
				sendMessage(s, messageTemp)
				
			#When command !results is typed, will print the URL for the results page for the BNF VCAs compiled by Tantusar 	
			if "!results" in message:
				messageTemp = "Check out the results for the Brave New Faves 2018 Viewers' Choice Awards at "
				messageTemp = messageTemp + "https://bravenewfaves.github.io/ "
				sendMessage(s, messageTemp)
				
			#When command !spotify is typed, will post link to spotify playlist of Kathleen's choices as compiled by pterodactal
			if "!spotify" in message:
				sendMessage(s, spotifylist)
				
			#When command !wiki is typed, will post link to the BNF wiki
			if "!wiki" in message:
				messageTemp = "Past playlists can be found on the BNF wiki: "
				messageTemp = messageTemp + "https://wiki.loadingreadyrun.com/index.php/Brave_New_Faves"
				sendMessage(s, messageTemp)
			...
			#Now any user can type !nowplaying to get the current song, but if I do it my bot breaks. :(
			if ("!nowplaying" in message):
				if ("kathleen_lrr" in user or "freshprinceofbeleren" in user):
					...
					if (len(playedlst) < 1):
						sendMessage(s, "No requests have been played yet")
					else:
						sendMessage(s, "Now playing request #" + playedlst[len(playedlst)])
				break			
			...
			if ("kathleen_lrr" in user or "freshprinceofbeleren" in user) and "!printlist" in message:
				with io.open(reqlist, 'w', encoding="utf-8") as f:
					for line in lst:
						f.write(("%s\n\n" % line))
					f.close()
				break
			...
			if ("kathleen_lrr" in user or "freshprinceofbeleren" in user) and "!printplaylist" in message:
				with io.open(playlist, 'w', encoding="utf-8") as f:
					for line in playedlst:
						f.write(("%s\n\n" % line))
					f.close()
				break
				
			#This lets pterodactal set the spotify playlist link for Kathleen's picks.
			if ("pterodactal" in user and "!set_playlist" in message):
				spotifylist = "Tonight's playlist can be found here: "
				spotifylist += message[13:]
